			...
			void line(BufferedImage buf, int x0, int y0, int xn, int yn, int rgb) {
				...
				*/
				...
				if (steep){
					...
					xn = yn;
					yn = tmp;
					System.out.println("two points get swapped");
				}
				if (x0>xn){
					int tmp;
					tmp = x0;
				...
				int deltax = xn - x0;
			    int deltay = Math.abs(yn - y0);
			    System.out.println("deltax & deltay:"+deltax+"&"+deltay );
			    int error = deltax/2;
			    int ystep;
			    int y = y0;
			    if (y0 < yn)  ystep = 1; else ystep = -1;
			    buf.flush();
			    for (int x = x0;x<=xn; x++){
			    	System.out.println("we are in the loop!");
			    	System.out.println("coo-width:"+buf.getWidth()+"coo-height:"+buf.getHeight());
			    	System.out.println("x:"+x+"y:"+y);
			    	if (steep) {buf.setRGB(Math.round(x), Math.round(y), rgb);
			    	 buf.flush();}
			    	else buf.setRGB(Math.round(y), Math.round(x), rgb);
		            error = error - deltay;
		            if (error < 0) {
		            	y = y + ystep;
		                error = error + deltax;
